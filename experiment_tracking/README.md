...
mistaken for the current queue.
...
Every authoritative score/result CSV must expose original evidence either in
row-level absolute cluster-log columns or through a named companion ledger.
`result_provenance_index_20260902.csv` is the current coverage index. A summary
without such a pointer is presentation-only and must not be used as the sole
scientific record. Explicitly absent historical MCTS evidence is recorded as
absence in `stage2_mcts_historical_log_audit_20260902.csv`; it must never be
silently converted into a zero score or a presumed completed job.

`result_provenance_audit_20260902.csv` is the structural audit of every
score-bearing CSV under this directory. It is regenerated by
`../scripts/audit_result_provenance.py`. The audit must contain no
`needs_mapping` rows before a score table is treated as authoritative.
...
The latest live status is `status_20260903_latest.md`. Its queue totals come
from `cluster_workload_latest.csv`; running-job evidence comes from
`live_training_progress_latest.csv` and `live_mcts_progress_latest.csv`.
`held_experiment_priority_20260903.csv` is the authoritative release order for
design-held work. The earlier order in prose was not a priority ranking.

MPrime validation adequacy is tracked separately under
`mprime_validation_ipc_scale_v1/validation_adequacy_test_plan_20260903.md`.
The existing corrected validation set must not be described as adequate merely
because it improved on the old epoch-1-biased set: Stage-1 rank correlation is
weak and all validation-led Stage-2 lineages select epoch 0. The test protocol
