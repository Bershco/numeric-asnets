# Live submission update — 14 September 2026, 00:21 IDT
...
Compute smoke `21233924` completed in 1m15s and passed 23 tests, CLI checks, the complete 20-row manifest, and all checkpoint existence checks. Four scientific tasks are terminal and 16 are running. The two completed matched pairs are exact ties: seed `1073581256` scores 59/59 under both rules, and seed `2011206605` scores 20/59 under both rules. This is neutral two-pair evidence, not a domain-wide effect estimate. Across all live ledgers, action-ID has classified 414/590 seed-instances with at least 254 successes; policy-prior has classified 409/590 with at least 249 successes. Those aggregate live values are lower bounds and must not be paired until every matching seed is terminal.
...
- at most 120 CPUs and 2,400 GiB if all tasks run concurrently.

Historical comparable Counters jobs have a median whole-job runtime near 49.9 hours, but the live per-arm aggregate cannot identify the slowest task well enough for a defensible point estimate. At this snapshot the 16 live tasks had run about 11.1 hours; their scheduler hard remaining bounds were about 60.6-60.9 hours. The array directly tests whether the candidate recovers the main RQ2 VH-off downgrade. Q-only final tie-breaking was not promoted because it recovered none of the three causal diagnostic instances.
...
At this snapshot, 469/588 checkpoint validations are complete. All original attempts have left the queue. Six resumable replacement tasks are live, requesting 24 CPUs and 120 GiB in total, with about 12h26m to their 24-hour hard bounds. Recheck controller `21233927` remains dependency-pending and will skip every completed point.
...
The Phase-B-A Stage-1 checkpoints are already final. An exact identity audit found 0/20 reusable Stage-1 fixed-MCTS evaluations because historical MCTS checkpoints did not match the final Phase-B-A selector. Compute smoke `21237283` solved its real MPrime instance and passed VAL. The two ten-task arrays `21237328` (VH-off) and `21237329` (VH-on) are running, requesting 120 CPUs and 2,400 GiB total. Every task uses normal fixed 20/70 search, terminal-safe action selection disabled, 6 CPUs, 120 GiB, a six-hour per-instance limit and a 72-hour hard allocation. At about 5h31m elapsed, their durable success lower bounds are at least 7.2/20 VH-off and 7.7/20 VH-on; no seed is terminal, so these are not final scores and no CI or p-value is valid. Three workers classify at most three instances concurrently, so twenty six-hour timeouts require seven waves: the evaluator-derived total bound is roughly 42 hours plus overhead, while the scheduler hard remainder is about 66h29m.

The matched MPrime Stage-1 PW70 screen is now live as `21240256[0-3]`: two predeclared seeds × two VH modes, requesting 24 CPUs and 480 GiB total. It uses the exact Phase-B-A checkpoints and fixed-arm settings, changing only to PW70 with `Kmin=3`, `c=.6`, and `alpha=.5`. All four tasks entered `RUNNING`; no scientific score was available after six minutes. The real compute smoke `21240250` solved 1/1 and passed VAL. Stage-2 PW remains gated on canonical Stage-2 endpoints and its fixed comparator.

## Block Grouping tie-break diagnostic

A two-job same-build screen, array `21237401`, tested the four exact Stage-1 VH-off seed `1963100312` instances that policy solved and historical narrow MCTS drove to 10,000 actions. It compared action-ID against policy-prior final tie-breaking with one worker, 2 CPUs, 120 GiB and 30 hours per task. Both jobs completed successfully after 4m55s and 5m11s. Both rules scored 0/4; every target ended as an ordinary 10,000-action unsolved trajectory, not an evaluation timeout. The screen is therefore a completed negative transfer result: policy-prior tie-breaking did not rescue this selected Block Grouping seed, and no wider fixed-search confirmation is justified from it. The real compute smoke passed 23 tie-break tests; only the exact node `ise-cpu-intl-13` was excluded after native exit `-4`.
...
The live scientific workload is 264 CPUs and 4,920 GiB: Counters strict confirmation 96/1,920; MPrime Stage-1 fixed MCTS 120/2,400; MPrime Stage-1 PW70 24/480; MPrime anchor recovery 24/120. One 1-CPU/2-GiB controller is dependency-pending. Every unambiguous primary prerequisite is live or controller-covered. MPrime Stage-2 remains gated on the anchor review. The completed negative Block Grouping screen is no longer consuming resources.
