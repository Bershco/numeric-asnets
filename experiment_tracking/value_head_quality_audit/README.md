...
### Quota-safe canonical chain — 19 September 2026, 12:08 IDT

The operational revision above exposed the underlying launch problem: Python
imports immediately regenerated bytecode caches, exhausted the shared-home
quota again and caused repeated `user env retrieval failed` holds.  The first
MPrime replacements reached TensorFlow initialization but then failed when the
quota closed.  Commit `07deb934` sets `PYTHONDONTWRITEBYTECODE=1` in every V1
capture and downstream science job, including inside the capture container.
After deleting only regenerated bytecode caches, the isolated checkout stayed
at zero `__pycache__` directories while all replacements ran for more than one
minute.

The superseded held FO jobs were cancelled.  Rover job `21463889_0` was kept
because it had already produced fresh bounded Stage-2 capture records.  The
canonical exact recovery identities are now:

- FO random: `21463988_[0-1]`, 96 GiB and four hours per seed;
- FO Stage-2 policy: `21463990_1`, 96 GiB and four hours;
- MPrime random plus both policy sources: `21463991_[0-1]`, 120 GiB and eight
  hours per seed;
- Rover Stage-2 policy: existing productive job `21463889_0`, 96 GiB and four
  hours.

All six identities were verified as genuinely running, not merely marked
`RUNNING`; Rover had fresh durable JSONL records and the bytecode-cache count
remained zero.  The final canonical dependency chain is
`21463994_[0-7]` -> `21463997` -> `21463998`.  It performs the eight manifest
