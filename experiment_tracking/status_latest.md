...
| MCTS first-divergence audit | Corrected smoke `21453647` passed on commit `a46e0a84`. Reconciliation preserved/materialized 23 outcomes without rerun and submitted exactly 17 recoveries across seven source-task groups. At least 31/40 candidates are terminal; FO-PW is complete at 6/6, and nine fixed-search candidates were active at the last result-ledger audit. | Four remaining array tasks allocate 8 CPU / 480 GiB; 26h task limits are conservative sequential-group bounds |
...
allocates 8 CPU / 480 GiB, while the repaired V1 smoke is resource-pending:
64 CPU / 1,264 GiB total. Dependency-pending arrays and controllers allocate nothing
