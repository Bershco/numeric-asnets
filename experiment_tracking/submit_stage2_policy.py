#!/usr/bin/env python3
"""Idempotently submit policy-only Stage-2 curves from a materialized manifest."""
...
def validate(rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        if row["status"] != "ready" or row["task_type"] != "policy_eval":
...
def submit(row: dict[str, str], dry: bool, suffix_prefix: str, output_prefix: str) -> str:
    domain = row["domain"]
    suffix = f"{suffix_prefix}_src{row['source_training_job_id']}_e{int(row['snapshot_epoch']):04d}"
    command = [
        ...
        "--job-suffix", suffix,
        "--output-subdir", f"{output_prefix}_{domain}",
    ]
...
def main() -> None:
    ...
    for row in representatives.values():
        submit(row, True, args.suffix_prefix, args.output_prefix)
