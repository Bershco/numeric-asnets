...
def broadcast_to(pattern, array):
    ...
    with tf.name_scope('broadcast_to'):
        pat_shape = tf.shape(input=pattern)
        arr_shape = tf.shape(input=array)
        denom = tf.where(
            tf.equal(pat_shape, 0), tf.ones_like(arr_shape), arr_shape)
        multiples = tf.math.floordiv(pat_shape, denom)
        rv = tf.tile(array, multiples)
        # now check shape matches
        rv_shape = tf.shape(input=rv)
        shapes_equal = tf.reduce_all(input_tensor=tf.equal(pat_shape, rv_shape))
...
def masked_softmax(activations, mask, name='masked_softmax'):
    ...
    with tf.name_scope('masked_softmax'):
        eq_size_op = tf.debugging.assert_equal(
            tf.shape(input=activations),
            tf.shape(input=mask),
        ...
        min_acts = tf.reduce_min(
            input_tensor=activations, axis=[-1], keepdims=True)
        ...
        max_acts = tf.reduce_max(
            input_tensor=disabled_min_out, axis=[-1], keepdims=True)
        ...
        any_valid = tf.reduce_any(
            input_tensor=mask, axis=[-1], keepdims=True, name='any_valid')
        ...
        with tf.name_scope('normalise'):
            ...
            sums = tf.reduce_sum(
                input_tensor=safe_exps, axis=-1, keepdims=True, name='sums')
...
def cross_entropy(act_dist, labels):
    ...
    with tf.name_scope('cross_entropy'):
        sums = tf.reduce_sum(input_tensor=act_dist, axis=-1, name='sums')
        all_good = tf.reduce_all(
            input_tensor=tf.abs(sums - 1) < 1e-2, axis=None, name='all_good')
        control_data = [
            sums, tf.reduce_min(input_tensor=sums, name='sums'),
            tf.reduce_max(input_tensor=sums, name='reduce_max_sums'),
            tf.reduce_all(input_tensor=tf.math.is_finite(sums), name='all_finite_sums')
        ]
...
def empty_feed_value(obs_dim, p_value_dim):
    ...
    new_obs_shape = [0, obs_dim]
    new_p_shape = [0, p_value_dim]
    shape_obs_prod = np.prod(new_obs_shape)
    shape_p_prod = np.prod(new_p_shape)
    assert shape_obs_prod == 0 and shape_p_prod == 0, \
