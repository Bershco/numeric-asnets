...
def main() -> None:
    ...
    for r in read(P / "pw70_confirmatory_seed_results_20260904.csv"):
        ...
        rows.append({
            **{k: r[k] for k in ("domain", "value_head", "seed")},
            ...
            "included_in_inference": "true", "job_state": "COMPLETED", "pw_job_id": "historical",
    ...
    for job_id, meta in new_jobs.items():
        ...
        rows.append({
            ...
            "included_in_inference": str(include).lower(), "job_state": d["state"], "pw_job_id": job_id,
    ...
    for (domain, vh), cell in sorted(groups.items()):
        ...
        item["status"] = "complete" if len(cell) == 10 else "n9_plus_one_oom_partial"
