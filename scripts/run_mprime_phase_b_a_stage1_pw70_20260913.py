#!/usr/bin/env python3
"""Run one exact row of the Phase-B-A MPrime Stage-1 PW70 screen."""
...
def load_row(path: Path, index: int) -> dict[str, str]:
    ...
    if len(rows) != 4:
        raise ValueError(f"manifest must contain exactly four rows, found {len(rows)}")
    ...
    for field, expected in EXPECTED.items():
        if row.get(field) != expected:
            raise ValueError(
                f"{row.get('manifest_id')}: {field}={row.get(field)!r}, expected {expected!r}"
            )
    if row.get("value_head") not in {"off", "on"} or not row.get("checkpoint"):
        raise ValueError("invalid value-head mode or missing checkpoint")
    return row
...
def experiment_arguments(
    ...
    args = [
        "./run_experiment",
        row["architecture_module"], row["domain_module"],
        "--resume-from", row["checkpoint"],
        "--eval-with-mcts",
        "--mcts-progressive-widening",
        "--mcts-pw-min-width", row["pw_min_width"],
        "--mcts-pw-c", row["pw_c"],
        "--mcts-pw-alpha", row["pw_alpha"],
    ...
    if row["terminal_safe"] == "true":
        args.append("--eval-mcts-terminal-safe-action-selection")
    if smoke_instance is not None:
        total = int(row["expected_test_instances"])
        if not 1 <= smoke_instance <= total:
            raise ValueError("smoke instance is outside the declared test set")
        skip = [str(number) for number in range(1, total + 1) if number != smoke_instance]
...
def main() -> int:
    ...
    experiment_args = experiment_arguments(row, completion, args.smoke_instance, args.smoke_timeout)
    inner = (
        "set -euo pipefail; "
    ...
    print(
        f"[MPRIME S1 PBA PW70] identity={identity} vh={row['value_head']} "
        f"seed={row['seed']} epoch={row['selected_epoch']} smoke={args.smoke_instance or 'no'}"
    )
    print("[MPRIME S1 PBA PW70] width=20 simulations=70 Kmin=3 c=.6 alpha=.5 puct=.1 estimator=.5 terminal_safe=false")
    print(f"[MPRIME S1 PBA PW70] checkpoint={row['checkpoint']} commit={actual_commit}")
    ...
    validation_rc = subprocess.run([
        ...
        "--log", str(log_path), "--domain", "mprime",
        "--validator", str(args.validator), "--allow-incomplete",
    ...
    append_attempt(output / "attempts.tsv", {
        ...
        "restart_count": restart, "started_at": started,
        "ended_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "evaluation_returncode": evaluation_rc,
        "validation_returncode": validation_rc, "log_path": log_path,
        "completion_record_path": completion, "val_summary_path": val_summary,
